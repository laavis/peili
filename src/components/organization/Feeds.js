...
export default ({ editable }) => {
  ...
  React.useEffect(() => {
    ...
    const storedIGFeeds = JSON.parse(localStorage.getItem('igFeeds') || '[]');
  ...
  React.useEffect(() => {
    // get feeds (or empty array if there are no feeds) from local storage
    const storedTwFeeds = JSON.parse(localStorage.getItem('twFeeds') || '[]');
    // display retrieved feeds on the screen
  ...
  const handleIGDelete = chipToDelete => () => {
    ...
    const chipIndex = newIGFeeds.findIndex(
      newIGFeeds => newIGFeeds === chipToDelete
    );
  ...
  const handleTwDelete = chipToDelete => () => {
    ...
    const chipIndex = newTwFeeds.findIndex(
      newTwFeeds => newTwFeeds === chipToDelete
    );
